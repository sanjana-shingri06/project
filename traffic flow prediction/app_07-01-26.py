...
# Real-time control flag (can be toggled via API)
...
def init_db():
    ...
    # Create alert rules table for configurable triggers
    cur.execute('''
...
def get_alerts_with_traffic(limit=20):
    ...
    # end get_alerts_with_traffic

def evaluate_rules(update):
    """Evaluate alert rules against a single update and create alerts when triggered."""
    try:
        conn = sqlite3.connect('traffic.db')
        cur = conn.cursor()
        cur.execute("SELECT id, road, metric, operator, threshold, severity, enabled FROM alert_rules WHERE enabled=1")
        rules = cur.fetchall()
        for rule in rules:
            rid, r_road, metric, operator, threshold, severity, enabled = rule
            try:
                # optional road scoping
                if r_road and r_road != update.get('road'):
                    continue
                ...
                trig = False
                if operator == '>':
                    trig = float(value) > float(threshold)
                elif operator == '<':
                    trig = float(value) < float(threshold)
                elif operator == '>=':
                    trig = float(value) >= float(threshold)
                ...
                if trig:
                    ...
                    alert = {
                        ...
                        'congestion_level': update.get('congestion_level'),
                        'vehicle_count': update.get('vehicle_count'),
                        'incident_type': f'Rule-{rid}'
                    }
                    try:
                        cur.execute('''
                            INSERT INTO traffic_alerts (road, severity, lat, lng, timestamp, congestion_level, vehicle_count, incident_type)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                        ''', (alert['road'], alert['severity'], alert['lat'], alert['lng'], alert['timestamp'], alert['congestion_level'], alert['vehicle_count'], alert['incident_type']))
                        conn.commit()
                        # broadcast
                        try:
                        ...
                        print(f"[ERROR] Failed to insert alert from rule {rid}: {e}")
            except Exception:
                continue
        conn.close()
    except Exception as e:
        print(f"[ERROR] evaluate_rules error: {e}")
...
def manage_rules():
    ...
    try:
        if request.method == 'GET':
            conn = sqlite3.connect('traffic.db')
            cur = conn.cursor()
            cur.execute('SELECT id, road, metric, operator, threshold, severity, enabled FROM alert_rules')
            rows = cur.fetchall()
            conn.close()
            rules = []
            for r in rows:
                rules.append({'id': r[0], 'road': r[1], 'metric': r[2], 'operator': r[3], 'threshold': r[4], 'severity': r[5], 'enabled': bool(r[6])})
        ...
        conn = sqlite3.connect('traffic.db')
        cur = conn.cursor()
        cur.execute('INSERT INTO alert_rules (road, metric, operator, threshold, severity, enabled) VALUES (?, ?, ?, ?, ?, ?)', (road, metric, operator, threshold, severity, enabled))
        conn.commit()
        rule_id = cur.lastrowid
        conn.close()
        return jsonify({'id': rule_id, 'road': road, 'metric': metric, 'operator': operator, 'threshold': threshold, 'severity': severity, 'enabled': bool(enabled)}), 201
    except Exception as e:
        print(f"[ERROR] manage_rules: {e}")
        return jsonify({'error': str(e)}), 500
...
def rule_detail(rule_id):
    try:
        ...
        data = request.get_json(force=True)
        # allow toggle or full update
        if 'enabled' in data:
            enabled = 1 if data.get('enabled') else 0
            cur.execute('UPDATE alert_rules SET enabled=? WHERE id=?', (enabled, rule_id))
        else:
            cur.execute('UPDATE alert_rules SET road=?, metric=?, operator=?, threshold=?, severity=?, enabled=? WHERE id=?', (
                data.get('road'), data.get('metric'), data.get('operator'), data.get('threshold'), data.get('severity', 'high'), 1 if data.get('enabled', True) else 0, rule_id
            ))
        conn.commit()
        conn.close()
        return jsonify({'updated': rule_id})
    except Exception as e:
        print(f"[ERROR] rule_detail: {e}")
        return jsonify({'error': str(e)}), 500


@app.route('/api/realtime/export', methods=['GET'])
def export_realtime_updates():
    """Export recent real-time updates as CSV"""
    try:
        ...
        # header
        writer.writerow(['road', 'vehicle_count', 'congestion_level', 'speed', 'timestamp', 'lat', 'lng'])
        for u in updates:
            writer.writerow([
                u.get('road', ''),
                u.get('vehicle_count', 0),
                u.get('congestion_level', 0),
                u.get('speed', 0),
                u.get('timestamp', ''),
                u.get('lat', ''),
                u.get('lng', '')
        ...
        return Response(contents, mimetype='text/csv', headers={
            'Content-Disposition': 'attachment; filename=live_updates.csv'
        })
    except Exception as e:
        print(f"[ERROR] Export realtime failed: {e}")
        return jsonify({'error': str(e)}), 500
...
controller = SmartTrafficController('traffic.db')

@app.route('/api/analytics/recommendations', methods=['GET'])
def get_recommendations():
    """Get AI-powered traffic management recommendations"""
    try:
...
@app.route('/api/analytics/road-health', methods=['GET'])
def get_road_health():
    """Get health scores for all roads"""
    try:
        health_scores = analytics.get_road_health_score()
        return jsonify(health_scores)
    except Exception as e:
        print(f"[ERROR] Health scores error: {e}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/analytics/anomalies', methods=['GET'])
def get_anomalies():
    """Detect and return traffic anomalies"""
    try:
        anomalies = analytics.detect_anomalies()
        return jsonify(anomalies)
    except Exception as e:
        print(f"[ERROR] Anomalies error: {e}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/analytics/forecast', methods=['GET'])
def get_forecast():
    """Get congestion forecast for next 6 hours"""
    try:
        hours = request.args.get('hours', 6, type=int)
        forecast = analytics.get_congestion_forecast(hours_ahead=hours)
        return jsonify(forecast)
    except Exception as e:
        print(f"[ERROR] Forecast error: {e}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/analytics/environmental-impact', methods=['GET'])
def get_environmental_impact():
    """Get environmental impact metrics"""
    try:
        impact = analytics.get_environmental_impact()
...
def get_peak_hours():
    """Get peak hours analysis per road"""
    try:
        peak_analysis = analytics.get_peak_hours_analysis()
        return jsonify(peak_analysis)
    except Exception as e:
        print(f"[ERROR] Peak hours error: {e}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/analytics/optimal-routes', methods=['GET'])
def get_optimal_routes():
    """Get optimal route recommendations"""
    try:
        source = request.args.get('source', 'Main Street')
        destination = request.args.get('destination', 'Broadway')
        routes = controller.calculate_optimal_routes(source, destination)
        return jsonify(routes)
    except Exception as e:
        print(f"[ERROR] Routes error: {e}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/analytics/auto-alerts', methods=['GET'])
def get_auto_alerts():
    """Get automatically generated traffic alerts"""
    try:
        alerts = controller.get_traffic_alerts_auto()
...
def get_analytics_summary():
    ...
    try:
        summary = {
            ...
            'auto_alerts': controller.get_traffic_alerts_auto(),
            'timestamp': datetime.now().isoformat()
        }
...
@app.route('/api/config/realtime', methods=['GET', 'POST'])
def config_realtime():
    """Get or set the realtime generator enabled flag"""
    global REAL_TIME_ENABLED
    try:
        if request.method == 'GET':
            return jsonify({'realtime': bool(REAL_TIME_ENABLED)})

        data = request.get_json(force=True)
        if 'realtime' not in data:
            return jsonify({'error': 'realtime field required'}), 400
        val = bool(data.get('realtime'))
        with REAL_TIME_LOCK:
            REAL_TIME_ENABLED = val
        print(f"[INFO] Real-time mode set to: {REAL_TIME_ENABLED}")
        return jsonify({'realtime': bool(REAL_TIME_ENABLED)})
    except Exception as e:
        print(f"[ERROR] config_realtime failed: {e}")
        return jsonify({'error': str(e)}), 500
...
if __name__ == "__main__":
    ...
    def real_time_data_generator():
        ...
        while True:
            # Respect global realtime enable flag so generator can be paused
            try:
                with REAL_TIME_LOCK:
                    enabled = REAL_TIME_ENABLED
            except Exception:
                enabled = True
            if not enabled:
                # When disabled, sleep briefly and continue checking flag
                time.sleep(1)
                continue
            try:
                ...
                for road in ROAD_COORDS.keys():
                    ...
                    # Also persist to traffic_data for historical analysis
                    try:
                        cur.execute('''
                            INSERT INTO traffic_data
                            (road, vehicle_count, timestamp, congestion_level, speed, incidents)
                            VALUES (?, ?, ?, ?, ?, ?)
                        ''', (road, vehicle_count, datetime.now().isoformat(), congestion, speed, 0))
                    except Exception:
                        # best-effort; don't break generator on persistence failure
                        pass
                    conn.commit()
                    conn.close()
                    ...
                    # Evaluate alert rules for this update
                    try:
                        evaluate_rules(update)
                    except Exception as e:
